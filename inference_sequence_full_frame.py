    ...
    for seq_name, seq_flist in tqdm(seq2images.items()):
        ...
        with torch.no_grad():
            for i, (img, metadata) in tqdm(enumerate(dataloader)):
                ...
                det_bboxes = np.concatenate((det_bboxes, np.array([[0,0,W_orig,H_orig]])), axis=0)
                # print(det_bboxes)
                ...
                if args.second_nms and img_out is not None:
                    ...
                    img_out, win_out = NMS(img_out, win_out, iou_thres=ith, redundant=args.redundant, merge=args.merge, 
                        max_det=args.max_det, agnostic=True)
