...
from trt8x import BaseEngine, run_det_trt, run_seg_trt
...
if __name__ == '__main__':
    ...
    net_det = BaseEngine(cfg_det["in_size"], engine_path=cfg_det['weights'], names=ds.classes, batch_size=det_batch)
    ...
    net_roi = BaseEngine(cfg_roi["in_size"], engine_path=cfg_roi['weights'], names=ds.classes, batch_size=1)
        ...
        with torch.no_grad():
            for i, (img, metadata) in tqdm(enumerate(dataloader)):
                ...
                if args.mode == 'track' and i < args.frame_delay: # detect (for frame_delay frames) to initialize the tracker
                    ...
                    det_dataloader = DataLoader(det_dataset, batch_size=det_batch, shuffle=False, num_workers=4) # all windows in a single batch

                    img_out = torch.empty((0, 6))
                    win_out = torch.empty((0, 4))

                    for j, (img_det, det_metadata) in enumerate(det_dataloader):
                        out, t = run_det_trt(net_det, img_det) #net_det(img_det.to(device))
                        T+=t
                    ...
                    seg_mask, t = run_seg_trt(net_roi, img)
                    T += t
                ...
                det_dataset =  WindowDetectionDataset(metadata['image_path'][0], ds, det_bboxes, cfg_det['in_size'])
                # det_dataloader = DataLoader(det_dataset, batch_size=len(det_dataset) if len(det_dataset)>0 else 1, shuffle=False, num_workers=4) # all windows in a single batch
                det_dataloader = DataLoader(det_dataset, batch_size=det_batch, shuffle=False, num_workers=4)
                ...
                for j, (img_det, det_metadata) in enumerate(det_dataloader):
                    ...
                    for si, pred in enumerate(out):
                        ...
                        pred[:,:4] = pred[:,:4] + det_metadata['translate'][si].to(pred.device) # to the coordinates of the original image
                    ...
                    if not img_out.numel():
                        ...
                        # print(img_out.shape)
                        img_out = torch.cat((img_out, torch.cat(out)), 0)      
                        # print(img_out.shape)  
                ...
                times.append(T) # seg time + all detector runs per image
