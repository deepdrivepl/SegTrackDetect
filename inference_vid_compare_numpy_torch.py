...
from datasets import ROIDataset, DATASETS
from datasets.dataset import WindowDetectionDatasetNumpy, WindowDetectionDatasetTorch
...
if __name__ == '__main__':
    ...
    for seq_name, seq_flist in tqdm(seq2images.items()):
        ...
        with torch.no_grad():
            for i, (img, metadata) in tqdm(enumerate(dataloader)):
                ...
                t1 = time.time()
                det_dataset_numpy =  WindowDetectionDatasetNumpy(img, metadata['image_path'][0], ds, det_bboxes, detector.input_size)
                det_dataset_torch =  WindowDetectionDatasetTorch(img.to(device), metadata['image_path'][0], ds, det_bboxes, detector.input_size)
                # det_dataset =  WindowDetectionDataset(img, metadata['image_path'][0], ds, det_bboxes, detector.input_size)

                img_det_numpy, det_metadata_numpy = det_dataset_numpy.get_batch()
                img_det_torch, det_metadata_torch = det_dataset_torch.get_batch()
                
                # print()
                # print()
                # print(img_det_numpy.shape, np.unique(img_det_numpy), img_det_numpy.dtype)
                # print(img_det_torch.shape, torch.unique(img_det_torch), img_det_torch.dtype)
                # exit()
                for k in det_metadata_numpy.keys():
                    eq = torch.equal(det_metadata_numpy[k], det_metadata_torch[k].cpu())
                    # print(k, eq)
                    if not eq:
                        print(k)
                        print(f'torch: {det_metadata_torch[k].cpu()}')
                        print(f'numpy: {det_metadata_numpy[k]}')
                        exit()
                # # exit()

                det_dataset = det_dataset_torch
                img_det, det_metadata = img_det_torch, det_metadata_torch
    ...
    # t_roi_coords, t_win_coords, t_pred, t_estim = roi_extractor.get_execution_times(all_images)
