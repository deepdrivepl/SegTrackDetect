...
from utils.general import create_directory, save_args
from utils.models import load_model
from utils.drawing import plot_one_box, get_colors, denormalize_numpy, draw_text
...
if __name__ == '__main__':
    ...
    # general
    parser.add_argument('--input_files', type=str, default='test-dev.txt')
    ...
    # save args
    out_dir = create_directory(args.out_dir)
    ...
    flist = sorted([x.rstrip() for x in open(cfg_ds["test_list"])])
    unique_sequences = list(set([x.split(os.sep)[cfg_ds["seq_pos"]] for x in flist]))
    ...
    for unique_sequence in unique_sequences:
        ...
        tracker = Sort(max_age = 1, min_hits = 3, iou_threshold = 0.3, min_confidence = 0.3)
        ...
        with torch.no_grad():
            for i, (img, metadata) in tqdm(enumerate(dataloader)):
                ...
                d0 = torch.squeeze(d0, (0,1))
                if not cfg_roi["sigmoid_included"]:
                    d0 = d0.sigmoid()
                d0 = torch.where(d0 > cfg_roi["thresh"], 1.0, 0.0)
                d0 = 255 * d0.detach().cpu().numpy().astype(np.uint8)
                # cv2.imwrite('test.png', d0)
                # exit()
                ...
                bboxes_det = getDetectionBboxes(
                    np.concatenate((roi_bboxes,trk_bboxes), axis=0), H_orig, W_orig, det_size=cfg_det['in_size'], bbox_type='sorted'
                )
                ...
                stats = ["ROI  %02d" % (len(roi_bboxes)), "MOT  %02d" % (len(trk_bboxes)), "DET  %02d" % (len(bboxes_det))]
                ...
                for j, (img_det, det_metadata) in enumerate(det_dataloader):
                    ...
                    out = net_det(img_det.to(device))[0]
