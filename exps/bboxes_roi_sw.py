...
def getDetectionBbox(bbox, max_H, max_W, det_size, padding=20, allow_resize=False):
    ...
    if (h_roi > h or w_roi > w) and not allow_resize:
        crop_bboxes = getSlidingWindowBBoxes(bbox, det_size, max_H, max_W)
        # print(crop_bboxes)
        return crop_bboxes #.tolist()
...
def getDetectionBboxesAll(bboxes, max_H, max_W, det_size, allow_resize):
    det_bboxes = []
    for bbox in bboxes:
        det_bboxes += getDetectionBbox(bbox,  max_H, max_W, det_size=det_size, allow_resize=allow_resize)
    return det_bboxes
    

# fix filtering - check roi_indices
def getDetectionBboxesNaive(rois, resize_det_windows, max_H, max_W, det_size, allow_resize):
    det_windows = []
    full_det_windows = []
    ...
    for i, roi in enumerate(rois):
        ...
        det_windows += getDetectionBbox(roi,  max_H, max_W, det_size=det_size, allow_resize=allow_resize)
        full_det_windows += [resize_det_windows[i]]
    return det_windows
...
def getDetectionBboxesSorted(rois, max_H, max_W, det_size, allow_resize):
    det_windows = []
    for i, roi in enumerate(rois):
        det_windows += getDetectionBbox(roi, max_H, max_W, det_size=det_size, allow_resize=True) # treat as if allow_resize=True (then len(windows)==len(rois))


    hits = {i: 0 for i in range(len(det_windows))}
    hit_indices = {i: [] for i in range(len(det_windows))}
    for i, det_window in enumerate(det_windows):
        hit_ = [isInsideBbox(roi, det_window) for roi in rois]
        roi_indices = np.where(np.array(hit_) == True)[0]
        print(roi_indices.tolist())

        hits[i]+=sum([isInsideBbox(roi, det_window) for roi in rois])
        hit_indices[i]=roi_indices.tolist()
    ...
    det_windows = [det_windows[i] for i in hits.keys()] # resized det_windows

    final_det_windows = getDetectionBboxesNaive(rois, det_windows, max_H, max_W, det_size=det_size, allow_resize=allow_resize)
    return final_det_windows 


# # check with both ROI sw and resize, it won't work with sw !
# def getDetectionBboxesSorted(bboxes, max_H, max_W, det_size, allow_resize):
#     _det_bboxes = []
#     for bbox in bboxes:
#         _det_bboxes += getDetectionBbox(bbox, max_H, max_W, det_size=det_size, allow_resize=allow_resize)

#     hits = {i: 0 for i in range(len(_det_bboxes))}
#     for i, _det_bbox in enumerate(_det_bboxes):
#         hits[i]+=sum([isInsideBbox(bbox, _det_bbox) for bbox in bboxes])

#     hits = OrderedDict(sorted(hits.items(), key=lambda item: item[1], reverse=True))
#     bboxes = [bboxes[i] for i in hits.keys()]

#     _det_bboxes = getDetectionBboxesNaive(bboxes, max_H, max_W, det_size=det_size, allow_resize=allow_resize)
#     return _det_bboxes 


# # just for fun xD
# def getDetectionBboxesSorted(rois, max_H, max_W, det_size, allow_resize):
#     def remove_and_sort(hit_indices, current_match):
#         for roi_i in hit_indices.keys():
#             for i in current_match:
#                 if i not in hit_indices[roi_i]:
#                     continue

#                 hit_indices[roi_i].pop(hit_indices[roi_i].index(i))

#         hits = {i: len(hit_indices[i]) for i in hit_indices.keys()}
#         hits_sorted = OrderedDict(sorted(hits.items(), key=lambda item: item[1], reverse=True))
#         return hits_sorted, hit_indices
...
#     all_roi_indices = [i for i in range(len(rois))]

#     while(len(all_roi_indices)) > 0:
#         roi_ind = list(hits_sorted.items())[0][0]
#         current_match = hit_indices[roi_ind]
#         roi_coords = rois[roi_ind]
        
#         final_det_windows += getDetectionBbox(roi_coords,  max_H, max_W, det_size=det_size, allow_resize=allow_resize)
#         for hitted_roi in current_match:
#             if hitted_roi in all_roi_indices:
#                 all_roi_indices.pop(all_roi_indices.index(hitted_roi))
#         hits_sorted, hit_indices = remove_and_sort(hit_indices, current_match)
