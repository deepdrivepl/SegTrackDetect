...
python inference_sequence.py --out_dir debug-errors/roi-track-fixes-resize-binary-trk-padding/Zebra \
--ds ZebraFish --flist data/3DZeF20/val.txt --name val \
--second_nms --mode 'roi_track' --merge  --binary_trk 

python inference_sequence.py --out_dir debug-errors/roi-track-fixes-resize-binary-trk-padding/SDS \
--ds SeaDronesSee --flist data/SeaDronesSee/test_dev.txt --name test_dev \
--second_nms --mode 'roi_track' --merge \
--roi_model unet-SDS1 \
--det_model yolov7_tiny_SDS \
...
python inference_sequence.py --out_dir debug-errors/roi-track-fixes-resize-binary-trk-padding/DC \
--ds DroneCrowd --flist data/DroneCrowd/test_minus_val.txt --name test_minus_val \
--second_nms --mode 'roi_track' --merge \
--roi_model unet-DC1 \
--det_model yolov7_tiny_DC \
