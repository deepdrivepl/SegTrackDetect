...
# avfter moving fix - move all
def getSlidingWindowBBoxes(win_bbox, det_size, max_H, max_W, padding = 20, overlap_px = 20):
    h,w = det_size

    xmin, ymin, xmax, ymax = win_bbox
    xmin, xmax = xmin-padding//2, xmax+padding//2
    ymin, ymax = ymin-padding//2, ymax+padding//2
...
# def getSlidingWindowBBoxes(win_bbox, det_size, max_H, max_W, overlap_px = 40):
#     h,w = det_size
...
# def getDetectionBbox(bbox, max_H, max_W, det_size, padding=10, allow_resize=False):
...
#         crop_bboxes = getSlidingWindowBBoxes(bbox, det_size)
#         # print(crop_bboxes)
...
def getDetectionBbox(bbox, max_H, max_W, det_size, padding=20, allow_resize=False):
    ...
    if (h_roi > h or w_roi > w) and not allow_resize:
        crop_bboxes, full_bbox = getSlidingWindowBBoxes(bbox, det_size, max_H, max_W, padding=padding)
        # print(crop_bboxes)
        return crop_bboxes, [full_bbox] #.tolist()
