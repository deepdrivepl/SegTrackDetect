...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_conf_th', type=float, default=0.1)
    ...
    for seq_name, seq_flist in seq2images.items():
        ...
        with torch.no_grad():
            for i, (img, metadata) in enumerate(dataloader):
                ...
                if args.mode == 'track' and i < args.frame_delay: # single det (for frame_delay frames) to initialize the tracker
                    ...
                    if args.debug:
                        ...
                        if os.path.basename(metadata['image_path'][0])=='31071.jpg':
                            xmin = min([max(int(x[0]-20),0) for x in det_bboxes])
                ...
                if 'track' in args.mode:
                    ...
                    if args.binary_trk:
                        ...
                        if seg_mask is not None:
                            ...
                            seg_mask = np.logical_or(seg_mask_fullres, mot_mask).astype(np.uint8)*255 
